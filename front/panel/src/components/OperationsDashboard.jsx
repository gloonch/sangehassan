...
export default function OperationsDashboard() {
  ...
  const load = async (isCancelled = () => false) => {
    const [a, w, s, p3] = await Promise.allSettled([
      ...
      fetchJSON("/api/v1/dashboard/phase3-summary")
    ]);
    ...
    setPhase3(p3.status === "fulfilled" ? p3.value.data : null);
  ...
  const regenerate = async () => { try { const response = await fetchJSON(`/api/v1/admin/customers/${matchedCustomer.id}/regenerate-activation`, { method: "POST" }); setActivation(response.data.activation_code); setCreatedWorkflow(null); } catch (e) { setError(e.message); } };
  ...
  return <div className="space-y-6" dir="rtl">
    {summary&&<section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[["فرایند فعال",summary.active_workflows],["در انتظار",summary.waiting_steps],["نیازمند تأیید",summary.approval_steps],["مرحله دیرکرد",summary.overdue_steps],["مغایرت بحرانی",summary.critical_discrepancies],["مغایرت باز",summary.open_discrepancies],["Task مسدود",summary.blocking_tasks],["بدون مسئول",summary.unassigned_steps],["نزدیک تحویل",summary.due_soon_workflows]].map(([label,value])=><article key={label} className="panel-card p-4"><strong className="text-2xl">{value}</strong><span className="mt-1 block text-xs text-primary/60">{label}</span></article>)}</section>}
    {phase3&&<section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[["بچ فعال",phase3.active_batches],["آماده ارسال",phase3.ready_for_shipment],["حمل فعال",phase3.active_shipments],["تحویل جزئی",phase3.partial_deliveries],["موجودی کم",phase3.low_stock],["رزرو منقضی",phase3.expired_reservations]].filter(([,value])=>value!==undefined).map(([label,value])=><article key={label} className="panel-card border-amber-200 p-4"><strong className="text-2xl">{value}</strong><span className="mt-1 block text-xs text-primary/60">{label}</span></article>)}</section>}
    <section className="panel-card"><h2 className="font-display text-2xl">اقدامات موردنیاز من</h2><p className="mt-1 text-sm text-primary/60">کارهای باز براساس نقش‌ها و دسترسی‌های شما</p>
    ...
    <section className="panel-card"><h2 className="font-display text-2xl">شروع فرایند جدید</h2><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{templates.map((w) => { const Icon = icons[w.icon_key] || Workflow; return <button key={w.id} onClick={() => choose(w)} className="rounded-2xl border bg-white/70 p-4 text-right transition hover:border-primary"><Icon className="mb-3 h-7 w-7"/><b>{w.name_fa}</b><p className="mt-1 text-xs text-primary/55">{w.description_fa}</p></button>; })}</div></section>
