...
SELECT 'Abbas Abad Travertine', 'تراورتن عباس آباد', 'ترافرتين عباس آباد', 'Cream travertine with soft motion.', 150000, '/images/travertine.jpg', c.id
FROM categories c
WHERE c.slug = 'travertine'
  AND NOT EXISTS (SELECT 1 FROM products WHERE title_en = 'Abbas Abad Travertine');

INSERT INTO products (title_en, title_fa, title_ar, description, price, image_url, category_id)
SELECT 'Natanzi Black Granite', 'گرانیت مشکی نطنز', 'جرانيت نطنز الأسود', 'Deep black granite with high durability.', 210000, '/images/granite.jpg', c.id
FROM categories c
WHERE c.slug = 'granite'
  AND NOT EXISTS (SELECT 1 FROM products WHERE title_en = 'Natanzi Black Granite');

INSERT INTO products (title_en, title_fa, title_ar, description, price, image_url, category_id)
SELECT 'White Marble', 'مرمر سفید', 'رخام أبيض', 'Classic white marble for bright interiors.', 260000, '/images/marble.jpg', c.id
FROM categories c
WHERE c.slug = 'marble'
