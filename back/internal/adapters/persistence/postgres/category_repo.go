...
func (r *CategoryRepository) List(ctx context.Context) ([]domain.Category, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, title_en, title_fa, title_ar, slug, created_at, COALESCE(updated_at, created_at)
		FROM categories
		ORDER BY id
	`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&category.ID, &category.TitleEN, &category.TitleFA, &category.TitleAR, &category.Slug, &category.CreatedAt, &category.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *CategoryRepository) GetByID(ctx context.Context, id int64) (domain.Category, error) {
	...
	row := r.db.QueryRowContext(ctx, `
		...
		WHERE id = $1
	`, id)
	if err := row.Scan(&category.ID, &category.TitleEN, &category.TitleFA, &category.TitleAR, &category.Slug, &category.CreatedAt, &category.UpdatedAt); err != nil {
...
func (r *CategoryRepository) Create(ctx context.Context, category domain.Category) (domain.Category, error) {
	...
	`, category.TitleEN, category.TitleFA, category.TitleAR, category.Slug)
...
func (r *CategoryRepository) Update(ctx context.Context, category domain.Category) (domain.Category, error) {
	row := r.db.QueryRowContext(ctx, `
		UPDATE categories
		SET title_en = $1, title_fa = $2, title_ar = $3, slug = $4, updated_at = NOW()
		WHERE id = $5
		RETURNING created_at, COALESCE(updated_at, created_at)
	`, category.TitleEN, category.TitleFA, category.TitleAR, category.Slug, category.ID)
