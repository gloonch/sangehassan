...
func (r *ProductTermRepository) List(ctx context.Context, taxonomy string) ([]domain.ProductTerm, error) {
	...
	if taxonomy == "" {
		rows, err = r.db.QueryContext(ctx, `
			SELECT id, taxonomy, term_key, label_en, label_fa, label_ar
			FROM product_terms
...
func (r *ProductTermRepository) Upsert(ctx context.Context, term domain.ProductTerm) (domain.ProductTerm, error) {
	row := r.db.QueryRowContext(ctx, `
		...
		DO UPDATE SET
		  ...
		  updated_at = NOW()
		RETURNING id, taxonomy, term_key, label_en, label_fa, label_ar
	`, term.Taxonomy, term.Key, term.LabelEN, term.LabelFA, term.LabelAR)

	var out domain.ProductTerm
	if err := row.Scan(&out.ID, &out.Taxonomy, &out.Key, &out.LabelEN, &out.LabelFA, &out.LabelAR); err != nil {
