...
func ensureBlogMetadata(db *sql.DB) error {
	...
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS blog_translations (
			...
			seo_title VARCHAR(255), seo_description TEXT, canonical_url TEXT,
			robots VARCHAR(40) NOT NULL DEFAULT 'index,follow',
			translation_status VARCHAR(20) NOT NULL DEFAULT 'draft',
			featured_image_alt TEXT, og_image_alt TEXT,
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ,
			UNIQUE (blog_id, locale), UNIQUE (locale, slug),
		...
		`CREATE INDEX IF NOT EXISTS idx_blogs_publication ON blogs (status, published_at, scheduled_at)`,
		`CREATE INDEX IF NOT EXISTS idx_blog_translations_public ON blog_translations (locale, translation_status, slug)`,
		`CREATE INDEX IF NOT EXISTS idx_blog_slug_redirects_lookup ON blog_slug_redirects (locale, old_slug)`,
		`INSERT INTO blog_translations (blog_id, locale, title, slug, excerpt, content_html, translation_status)
		SELECT b.id, 'en', b.title, 'legacy-blog-' || b.id, COALESCE(b.excerpt, ''), COALESCE(b.content, ''),
			CASE WHEN b.status = 'published' THEN 'published' ELSE 'draft' END
		FROM blogs b
		WHERE NOT EXISTS (SELECT 1 FROM blog_translations bt WHERE bt.blog_id = b.id)
