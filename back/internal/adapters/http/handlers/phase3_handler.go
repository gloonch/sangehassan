...
func idempotencyKey(c *gin.Context) (string, bool) {
	...
	if key == "" {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": "Idempotency-Key header is required", "code": "IDEMPOTENCY_KEY_REQUIRED"})
		return "", false
	}
	return key, true
}
...
func (h *OperationsHandler) OrderItems(c *gin.Context) {
	okOrError(c, operationResult(h.service.ListOrderItems(c.Request.Context(), c.Param("id"))))
}
...
func (h *OperationsHandler) Phase3Dashboard(c *gin.Context) {
	okOrError(c, operationResult(h.service.Phase3Dashboard(c.Request.Context(), actorID(c))))
}
