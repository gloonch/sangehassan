...
func (s *OperationsService) PrepareWorkflowFileUpload(ctx context.Context, actor, stepID string, fieldID int64) (string, bool, WorkflowUploadPolicy, error) {
	...
	if fieldType != "FILE" && fieldType != "IMAGE" && fieldType != "SIGNATURE" {
	...
	policy := WorkflowUploadPolicy{MaxSizeBytes: 15 << 20}
	if fieldType == "FILE" {
...
func (s *OperationsService) PrepareOperationalFileUpload(ctx context.Context, actor, entityType, entityID string, customerVisible bool) (string, bool, WorkflowUploadPolicy, error) {
	...
	case "SHIPMENT", "DELIVERY":
		err := s.db.QueryRowContext(ctx, `SELECT sh.workflow_instance_id,o.customer_user_id FROM shipments sh JOIN orders o ON o.id=sh.order_id WHERE sh.id=$1`, entityID).Scan(&workflowID, &owner)
		if err != nil {
			return "", false, WorkflowUploadPolicy{}, err
		}
		if owner == actor {
	...
	case "PACKAGE":
		err := s.db.QueryRowContext(ctx, `SELECT b.workflow_instance_id,o.customer_user_id FROM packaging_units p JOIN fulfillment_batches b ON b.id=p.batch_id JOIN orders o ON o.id=b.order_id WHERE p.id=$1`, entityID).Scan(&workflowID, &owner)
		if err != nil {
			return "", false, WorkflowUploadPolicy{}, err
		}
	...
	return workflowID, customerVisible, WorkflowUploadPolicy{AllowedMIMETypes: []string{"image/png", "image/jpeg", "application/pdf"}, MaxSizeBytes: 15 << 20}, nil
}
...
func (s *OperationsService) WorkflowFileForDownload(ctx context.Context, actor, fileID string) (WorkflowFile, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT f.id,f.workflow_instance_id,f.workflow_step_instance_id,f.field_definition_id,f.entity_type,f.entity_id,f.storage_key,f.original_file_name,f.mime_type,f.size_bytes,f.customer_visible,wi.customer_user_id FROM workflow_files f JOIN workflow_instances wi ON wi.id=f.workflow_instance_id WHERE f.id=$1`, fileID).Scan(&file.ID, &file.WorkflowInstanceID, &file.WorkflowStepInstanceID, &file.FieldDefinitionID, &file.EntityType, &file.EntityID, &file.StorageKey, &file.OriginalFileName, &file.MIMEType, &file.SizeBytes, &file.CustomerVisible, &customerID)
	if err != nil {
		return file, err
	}
	if actor == customerID {
