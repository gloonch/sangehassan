...
func (s *OperationsService) insertLotTx(ctx context.Context, tx *sql.Tx, actor string, p LotPayload, quantity, unit string) (InventoryLot, error) {
	...
	err = tx.QueryRowContext(ctx, `INSERT INTO inventory_lots(lot_number,parent_lot_id,origin_type,origin_reference_id,current_location_id,stone_category,stone_name,stone_variant,quality_grade,finish_type,cut_type,initial_quantity,available_quantity,reserved_quantity,quantity_unit,secondary_quantity,secondary_unit,status,received_at,created_by_user_id,supplier_id) VALUES($1,$2,$3,NULLIF($4,''),$5,$6,$7,$8,$9,$10,$11,$12::numeric,$12::numeric,0,$13,NULLIF($14,'')::numeric,NULLIF($15,''),'AVAILABLE',NOW(),$16,$17) RETURNING id`, number, p.ParentLotID, normalizeCode(p.OriginType), p.OriginReferenceID, p.LocationID, p.StoneCategory, p.StoneName, p.StoneVariant, p.QualityGrade, p.FinishType, p.CutType, quantity, unit, valueOrEmpty(p.SecondaryQuantity), p.SecondaryUnit, actor, p.SupplierID).Scan(&x.ID)
	if err != nil {
		return x, err
	}
	x.LotNumber = number
...
func (s *OperationsService) ConvertInventory(ctx context.Context, actor, key string, p ConversionPayload) (map[string]any, error) {
	...
	if p.SupplierID != nil {
		p.OutputLot.SupplierID = p.SupplierID
	}
	...
	err = tx.QueryRowContext(ctx, `INSERT INTO inventory_lot_conversions(operation_group_id,batch_id,workflow_step_instance_id,input_lot_id,output_lot_id,order_item_conversion_id,input_quantity,input_unit,output_quantity,output_unit,waste_quantity,waste_unit,conversion_type,created_by_user_id,supplier_id) VALUES($1,$2,$3,$4,$5,$6,$7::numeric,$8,$9::numeric,$10,$11::numeric,$12,$13,$14,$15) RETURNING id`, group, p.BatchID, p.WorkflowStepInstanceID, p.InputLotID, output.ID, p.OrderItemConversionID, p.InputQuantity, p.InputUnit, p.OutputQuantity, p.OutputUnit, p.WasteQuantity, p.WasteUnit, normalizeCode(p.ConversionType), actor, p.SupplierID).Scan(&conversionID)
	if err != nil {
		return nil, err
	}
