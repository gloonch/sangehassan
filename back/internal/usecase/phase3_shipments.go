...
func (s *OperationsService) CreatePackaging(ctx context.Context, actor, batchID, key string, p PackagingPayload) (map[string]any, error) {
	...
	err = tx.QueryRowContext(ctx, `INSERT INTO packaging_units(package_number,batch_id,inventory_lot_id,package_type,quantity,quantity_unit,gross_weight,net_weight,weight_unit,status,customer_visible) VALUES($1,$2,$3,$4,$5::numeric,$6,NULLIF($7,'')::numeric,NULLIF($8,'')::numeric,NULLIF($9,''),'PACKED',$10) RETURNING id`, number, batchID, p.InventoryLotID, normalizeCode(p.PackageType), p.Quantity, normalizeCode(p.QuantityUnit), valueOrEmpty(p.GrossWeight), valueOrEmpty(p.NetWeight), normalizeCode(p.WeightUnit), p.CustomerVisible).Scan(&id)
	if err != nil {
		return nil, err
	}
