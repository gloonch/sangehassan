...
func (s *OperationsService) validateCostEntity(ctx context.Context, entityType, entityID string) error {
	table := map[string]string{"ORDER": "orders", "BATCH": "fulfillment_batches", "SHIPMENT": "shipments", "INVENTORY_MOVEMENT": "inventory_movements"}[entityType]
	if entityType == "INSTALLATION" {
		return conflict("UNSUPPORTED_ENTITY", "installation entity is not available yet")
	}
	if table == "" {
...
func (s *OperationsService) CreateOperationalCost(ctx context.Context, actor string, p CostPayload) (map[string]any, error) {
	...
	if !validNonNegativeDecimal(p.Amount) || len(p.Currency) != 3 {
	...
	var id string
	err := s.db.QueryRowContext(ctx, `INSERT INTO operational_cost_entries(entity_type,entity_id,cost_type,amount,currency,notes,incurred_at,created_by_user_id) VALUES($1,$2,$3,$4::numeric,$5,NULLIF($6,''),$7,$8) RETURNING id`, p.EntityType, p.EntityID, p.CostType, p.Amount, p.Currency, p.Notes, incurred, actor).Scan(&id)
	if err != nil {
		return nil, err
	}
	s.audit(ctx, actor, "operational_costs.create", "operational_cost_entry", id, p)
	return map[string]any{"id": id, "status": "DRAFT"}, nil
}
...
func (s *OperationsService) ApproveOperationalCost(ctx context.Context, actor, id string) error {
	r, err := s.db.ExecContext(ctx, `UPDATE operational_cost_entries SET status='APPROVED',approved_by_user_id=$2,approved_at=NOW(),updated_at=NOW() WHERE id=$1 AND status='DRAFT'`, id, actor)
	if err != nil {
		return err
	}
...
func (s *OperationsService) VoidOperationalCost(ctx context.Context, actor, id, reason string) error {
	...
	r, err := s.db.ExecContext(ctx, `UPDATE operational_cost_entries SET status='VOID',voided_at=NOW(),void_reason=$2,updated_at=NOW() WHERE id=$1 AND status<>'VOID'`, id, reason)
	if err != nil {
		return err
	}
	if n, _ := r.RowsAffected(); n == 0 {
...
func (s *OperationsService) Phase3Dashboard(ctx context.Context, actor string) (map[string]any, error) {
	out := map[string]any{}
	queries := map[string]string{}
	if s.HasPermission(ctx, actor, "batches.view_all") {
	...
	} else if s.HasPermission(ctx, actor, "batches.view_assigned") {
		var n int
	...
	} else if s.HasPermission(ctx, actor, "shipments.view_assigned") {
	...
	if s.HasPermission(ctx, actor, "orders.view_all") {
	...
	if s.HasPermission(ctx, actor, "containers.view") {
